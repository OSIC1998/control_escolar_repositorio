...
Widget camposGeneralfecha(String Ncampo, TextEditingController textEditingController) {
  return Container(
      padding: EdgeInsets.symmetric(horizontal: 15, vertical: 5),
      child: TextFormField(
...
Widget camposGeneralNumeros(String Ncampo, TextEditingController textEditingController) {
  return Container(
      padding: EdgeInsets.symmetric(horizontal: 15, vertical: 5),
      child: TextFormField(
...
Widget selecctAlumnoSalon(BuildContext context, String nombre,TextEditingController CS, TextEditingController CIDS, TextEditingController CIDSSalones) {
  return GestureDetector(
    onTap: () async {
      if(CIDSSalones.text!="")
      {
            ...
            showDialog(context: context, builder: (context)=>AlertDialog(
              ...
              actions: <Widget>[
                FlatButton(onPressed: () async {
                  if(nombre.text==""||direccion.text==""||apellido.text==""||clave.text==""||correo.text==""||tipo.text==""||telefono.text==""||user.text==""||fecha.text=="")
                  {
                    showDialog(context: context, builder: (context)=>AlertDialog(
          ...
          if(MateriasIncritas.docs.length != 0){
            ...
            else
            {
              for(var MI in MateriasIncritas.docs)
              {
                ...
                if(notaAlum.docs.length!=0){
                  for(var NA in notaAlum.docs){
                    if(NA.id==MI.get(bd.campo_tabla_materia_inscripta_id_nota)){
                      ...
                      notas=nota1.toString()+", "+nota2.toString()+", "+nota3.toString()+", "+nota4.toString()+", "+nota5.toString();
                ...
                QuerySnapshot materiaAlum= await B.buscarDatos(bd.tabla_materia);
                if(materiaAlum.docs.length!=0){
                  for(var MA in materiaAlum.docs){
                    if(MA.id==MI.get(bd.campo_tabla_materia_inscripta_id_materia)){
                      materia=MA.get(bd.campo_tabla_materia).toString();
                    }
                  }
                }
                dosente="sin resultados";
                QuerySnapshot DosenteAlum= await B.buscarDatos(bd.tabla_docente);
                if(DosenteAlum.docs.length!=0){
                  for(var DA in DosenteAlum.docs){
                    if(DA.id==MI.get(bd.campo_tabla_materia_inscripta_id_docente)){
                      dosente=DA.get(bd.campo_tabla_docente_nombre).toString()+" "+DA.get(bd.campo_tabla_docente_apellido);
                    }
                  }
                }
            ...
            showDialog(context: context, barrierDismissible: false, builder: (context)=>AlertDialog(
              title: Text("alunnos"),
              content: filtroNotas(context, listaFiltradaNombre, listaFiltradaNotas, listaFiltradaNotaID, listaFiltradaNota1, listaFiltradaNota2, listaFiltradaNota3, listaFiltradaNota4, listaFiltradaNota5, listaFiltradapromedios, listaFiltradaMateria, listaFiltradaDosente),
              actions: <Widget>[
                FlatButton(onPressed: (){Navigator.of(context).pop();}, child: Text("CERRAR"))
              ],
            ));
...
Widget botonMostrarReporte(BuildContext context,TextEditingController idAlumno){
  return Container(
    width: double.infinity,
    child: MaterialButton(
      color: Color.fromARGB(255, 26, 223, 4),
        ...
        onPressed: () async {
          ...
          if(MateriasIncritas.docs.length != 0){
            ...
            else
            {
              for(var MI in MateriasIncritas.docs)
              {
                if(MI.get(bd.campo_tabla_materia_inscripta_id_alumno)==idAlumno.text)
                {
                  NombreA="sin resultados";
                  QuerySnapshot alumno= await B.buscarDatos(bd.tabla_tablaalumno);
                  if(alumno.docs.length!=0){
              ...
              showDialog(context: context, barrierDismissible: false, builder: (context)=>AlertDialog(
                ...
                content: filtroNotas(context, listaFiltradaNombre, listaFiltradaNotas, listaFiltradaNotaID, listaFiltradaNota1, listaFiltradaNota2, listaFiltradaNota3, listaFiltradaNota4, listaFiltradaNota5, listaFiltradapromedios, listaFiltradaMateria, listaFiltradaDosente),
                actions: <Widget>[
                  FlatButton(onPressed: (){Navigator.of(context).pop();}, child: Text("crear PDF")),
                  FlatButton(onPressed: (){Navigator.of(context).pop();}, child: Text("cerrar")),
                ],
              ));
            }
...
Widget filtroNotas(BuildContext context, List<String> listaFiltradaNombre, List<String> listaFiltradaNotas,List<String> listaFiltradaNotaID,List<double> Nota1,List<double> Nota2,List<double> Nota3,List<double> Nota4,List<double> Nota5, List<double> listaFiltradaPromedios, List<String> listaFiltradaMateria, List<String> listaFiltradaDosente){
  return Container(
    width: double.infinity,
    child: SingleChildScrollView(
      child: Column(
              ...
              children: List.generate(listaFiltradaNombre.length, (index){
                return Padding(
                  ...
                  child: GestureDetector(
                    onTap: (){
                      showDialog(context: context, builder: (context)=>AlertDialog(
                        content: Modificar_nota(context,listaFiltradaNombre[index],listaFiltradaNotaID[index], Nota1[index], Nota2[index], Nota3[index], Nota4[index], Nota5[index]),
                        actions: <Widget>[
                          FlatButton(onPressed: (){Navigator.of(context).pop();}, child: Text("canselar"))
                        ],
                      ));
                        ...
                        child: Column(
                          children: [
                            ...
                            Container(
                              child: Padding(
                                padding: const EdgeInsets.all(5),
                                  child: Text("MATERIA", style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600),),
                              ),
                            ),
                            Container(
                              child: Padding(
                                padding: const EdgeInsets.all(5),
                                child: Text(listaFiltradaMateria[index], style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600),),
                              ),
                            ),
                            Container(
                              child: Padding(
                                padding: const EdgeInsets.all(5),
                                  child: Text("DOSENTE", style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600),),
                              ),
                            ),
                            Container(
                              child: Padding(
                                padding: const EdgeInsets.all(5),
                                child: Text(listaFiltradaDosente[index], style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600),),
                              ),
                            ),
                            Container(
                              child: Padding(
...
Widget Modificar_nota(BuildContext context,String nombre, String ID, double N1, double N2, double N3, double N4, double N5){
  ...
  return Column(
    children: <Widget>[
      Center(
        child: Text("ALUMNO: "+nombre,style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600),),
      ),
              ...
              try{
                ...
                if(n1!=null||n2!=null||n3!=null||n4!=null||n5!=null){
                  int R= await m.ModificarNota(ID, n1, n2, n3, n4, n5);
                    if(R==1)
                    {
                      mensage="nostas modificado con éxito";
                    }
                    else{
                    ...
                    actions: <Widget>[
                      FlatButton(onPressed: (){Navigator.pushAndRemoveUntil(context,MaterialPageRoute(builder: (BuildContext context) => menuapp(),),(route) => false,);}, child: Text("OK")),
                    ],
                  ));
                }
              }
                ...
                showDialog(context: context,builder: (context)=>AlertDialog(
                  title: Text("error"),
                  content: Text("Por favor colocar solo números: "+e.toString()),
                  actions: <Widget>[
                    FlatButton(onPressed: (){Navigator.of(context).pop();}, child: Text("OK"))
                  ],
                ));
